...
describe('MessageList', () => {
  ...
  it('forwards onSkip callbacks to the active row loading indicator and show work components', () => {
    const onSkip = vi.fn();
    const liveWork: Work = {
      results: {
        [STEPS.INITIAL]: ['draft'],
      ...
      agentStates: [createAgentState({ messageId: 'model-1', status: 'error', label: 'Draft Failed' })],
    };
    const messages: Message[] = [
      {
        id: 'model-1',
    ...
    expect(rowLoadingProps.onSkip).toBe(onSkip);
    expect(rowShowWorkProps.onSkip).toBe(onSkip);

    rowLoadingProps.onSkip();
    expect(onSkip).toHaveBeenCalledTimes(1);

    onSkip.mockClear();
    rowShowWorkProps.onSkip();
    expect(onSkip).toHaveBeenCalledTimes(1);
  });
