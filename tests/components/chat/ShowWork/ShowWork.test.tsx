...
describe('ShowWork', () => {
  ...
  it('shows Skip Step for recoverable errors on non-synthesis steps and calls onSkip', () => {
    const onSkip = vi.fn();
    mocks.store.activeSessionMessageId = 'message-1';
    mocks.store.sessionsByMessageId = {
      'message-1': {
        work: createWork({
          stepMetadata: [{ id: STEPS.REFINEMENT, status: 'error', label: 'Refinement Step' }],
        }),
        agentStates: [
          createAgentState({
            id: 'critic-1',
  ...
  it('hides Skip Step when synthesis step is in error status and keeps Retry visible', () => {
    const onRegenerate = vi.fn();
    mocks.store.activeSessionMessageId = 'message-1';
    mocks.store.sessionsByMessageId = {
      'message-1': {
